/**
 * Gallery.tsx — Gallery Section ("drifting print-desk")
 *
...
import {
  ...
  AnimatePresence, useReducedMotion, type MotionValue,
} from 'framer-motion'
...
// small viewports.
type Tier = 'S' | 'M' | 'L'
interface Spot { x: number; y: number; t: Tier }
...
function pieceWidth(t: Tier, ratio: number, isMobile: boolean) {
  return isMobile
...
function FloatingPiece({
  ...
  const [constraints, setConstraints] = useState<DragBounds | null>(null)
...
function ViewAllContent({ showSoon, onHover }: { showSoon: boolean; onHover: (v: boolean) => void }) {
  return (
    <div
...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const check = ({ scroll }: { scroll: number }) => {
  ...
  const layerOpacity = useTransform([layerEntryO, layerExitO], ([inO, outO]: number[]) => inO * outO)
  ...
  const reduceMotion = useReducedMotion()
  const inkT = useTransform([revealRaw, layerExitO], ([r, o]: number[]) => r * o)
  ...
  const stagePointerEvents = useTransform(
    [revealRaw, progress],
    ([r, p]: number[]) => (r > 0.6 && p < 0.85) ? 'auto' : 'none'
  )
