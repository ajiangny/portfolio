// src/components/gradient/three/FluidScene.ts
...
import { Simulation, type PointerState } from './Simulation'
import { ShaderPass } from './ShaderPass'
import { VERT, COMPOSITE } from './shaders'
import { SIM, GRADIENT } from '../gradientConfig'
import type { SelectedPalette } from '../paletteSelect'
...
export class FluidScene {
  ...
  simRes: number
  // Assigned only when WebGL2 + float render targets are available; every
  // method that touches them is gated on `supported`.
  sim!: Simulation
  ...
  setSimRes(res: number) { this.simRes = res }

  resize(w: number, h: number, dpr: number) {
    if (!this.supported || !this.renderer) return
  ...
  _applyPalette(palette: SelectedPalette) {
  ...
  update({ pointer, ambient, time, iters, palette }: UpdateOptions) {
  ...
  prewarm({ steps, iters, palette }: PrewarmOptions): Promise<void> {
    ...
    const pointer: PointerState = { x: -1, y: -1, dx: 0, dy: 0, down: false }
    let done = 0
  ...
  renderStatic(palette: SelectedPalette) {
    if (!this.supported) return
    this._applyPalette(palette)
    this.composite.render(null)
  }
