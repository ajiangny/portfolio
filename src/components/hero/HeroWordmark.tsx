/**
 * HeroWordmark.tsx — "ANDREW JIANG" wordmark (vector) with elastic letters.
 *
 ...
 * shared filter (hooks/useInkFilter.tsx) — same turbulence-displacement +
...
import {
  ...
  useMotionTemplate, useInView, animate, type MotionValue,
} from 'framer-motion'
...
function repel(mx: number, my: number, cx: number, cy: number, axis: 'x' | 'y') {
  if (mx === OFF) return 0
  const dx = mx - cx
  const dy = my - cy
...
interface WordmarkLetterProps {
  d: string
  mouseX: MotionValue<number>
  mouseY: MotionValue<number>
}

function ElasticLetter({ d, mouseX, mouseY }: WordmarkLetterProps) {
  ...
  const rawX = useTransform([mouseX, mouseY], ([mx, my]: number[]) => repel(mx, my, cx.current, cy.current, 'x'))
  const rawY = useTransform([mouseX, mouseY], ([mx, my]: number[]) => repel(mx, my, cx.current, cy.current, 'y'))
  const x = useSpring(rawX, SPRING)
  const y = useSpring(rawY, SPRING)

  // Shadow alpha: 0 when cursor is absent or outside radius, peaks at ~0.55 on direct hover.
  const shadowAlpha = useTransform([mouseX, mouseY], ([mx, my]: number[]) => {
    if (mx === OFF) return 0
...
export default function HeroWordmark({ className, style, title = 'Andrew Jiang' }: HeroWordmarkProps) {
