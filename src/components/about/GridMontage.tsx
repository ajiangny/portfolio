/**
 * GridMontage.tsx — About Section Gallery Cluster
 *
...
import {
  motion, useTransform, useMotionValue, useReducedMotion, animate,
  type MotionValue, type AnimationPlaybackControls,
} from 'framer-motion'
...
const REVEAL_EASE: [number, number, number, number] = [0.215, 0.61, 0.355, 1] // power3.out — matches the ink reference
...
const pick = <T,>(arr: T[], r: number): T => arr[Math.min(r, arr.length - 1)]
...
interface CellProps {
  ...
  layout: CellLayout
  isProfile: boolean
  profileHideOpacity: MotionValue<number>
  cellRef?: RefObject<HTMLDivElement | null>
  reduced: boolean | null
}

function Cell({ src, progress, layout, isProfile, profileHideOpacity, cellRef, reduced }: CellProps) {
  ...
  const setOuterRef = useCallback((node: HTMLDivElement | null) => {
  ...
  const inkT = useTransform([reveal, dissolve], ([rv, ds]: number[]) =>
  ...
  const opacity = useTransform([inkOpacity, profileHideOpacity], ([ink, hide]: number[]) =>
...
export interface GridMontageProps {
  images: string[]
  cols: number
  profileIndex: number
  progress: MotionValue<number>
  profileCellRef?: RefObject<HTMLDivElement | null>
  profileHideOpacity: MotionValue<number>
  /** Max horizontal fraction of the viewport (0–100). */
  fitVw?: number
  /**
   * Vertical safe zone in px (navbar clearance + parent padding) — must match
   * About.tsx's wrapper padding so the cluster never overflows under the nav.
   */
