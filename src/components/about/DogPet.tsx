/**
 * DogPet.tsx — interactive pixel-art pet for the tagline tile
 *
...
type Sheet = (typeof SHEETS)[SheetName]

export interface DogPetProps {
  /** number → fixed size; undefined → responsive */
  size?: number
  jumpScale?: number
  minSize?: number
  maxSize?: number
  /** dog ≈ 36% of tile width (≈300px at a 1920-wide layout) */
  widthFactor?: number
  /** …but never taller than 55% of the tile (guards short screens) */
  heightFactor?: number
  /** one-time fall-in from off-screen above (Contact instance only) */
  spawnDrop?: boolean
}
...
export default function DogPet({
  ...
  const layerRef = useRef<HTMLDivElement | null>(null) // the inset-0 stage; its size = tile interior
  const dogRef = useRef<HTMLButtonElement | null>(null) // the moving sprite (a <button>)
  const spriteRef = useRef<HTMLDivElement | null>(null) // the inner sprite sheet
  ...
  useEffect(() => {
    ...
    const applySpriteAnim = (sheet: Sheet) => {
    ...
    const st: {
      ...
      x: NaN, // px from left of stage; NaN until first frame seeds it
    ...
    const pickBehaviour = (now: number) => {
      const maxX = bounds()
      const canWalk = maxX > 12
      const r = Math.random()
      let mode: typeof st.mode
    ...
    const loop = (now: number) => {
      ...
      const SIZE = sizeRef.current
      const maxX = bounds()
      if (Number.isNaN(st.x)) st.x = clamp(widthRef.current * 0.22, 0, maxX)
