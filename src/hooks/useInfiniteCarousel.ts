/**
 * useInfiniteCarousel.ts — Infinite Project Carousel Logic
 *
 ...
 *
 * @param scrollRef - the scrollable container holding 3×itemCount children
 * @param itemCount - number of unique items (one set)
 * @param isMobile - true below the 768px breakpoint
 */
import { useEffect, useRef, useState, type RefObject } from 'react'
import { animate, type AnimationPlaybackControls } from 'framer-motion'

export interface InfiniteCarousel {
  /** index into the 3×runway of the centred card */
  activeIndex: number
  /** true once the user moved the carousel (mobile hint) */
  hasSwiped: boolean
  /** animate card i to the centre */
...
export default function useInfiniteCarousel(
  ...
  const animationRef = useRef<AnimationPlaybackControls | null>(null) // tracks active FM scroll animation
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const target = el?.children[activeIndexRef.current] as HTMLElement | undefined
      if (!el || !target) return
      if (isMobile) {
  ...
  const handleScroll = () => {
    ...
    ;(Array.from(el.children) as HTMLElement[]).forEach((child, index) => {
    ...
    scrollTimeoutRef.current = setTimeout(() => {
      ...
      const firstChild = currentEl.children[0] as HTMLElement | undefined
      const setFirstChild = currentEl.children[itemCount] as HTMLElement | undefined
